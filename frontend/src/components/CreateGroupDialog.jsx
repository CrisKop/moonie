...
export default function MyModal(props) {
  const {CreateGroupDialogOpen,  setCreateGroupDialogOpen, createChat, userChats } = useChat();
  const {user} = useAuth();
  ...
  const {CurrentChatData, setCurrentChatData, LoadingCreateChat } = useChat();
 // const {dialogButtons} = useDash();
  ...
  const createChatFunc = async () => {
    ...
    if(currentSelect.length < 2) return;

    if(window.innerWidth < 1270){
      setLeftBarShow(!LeftBarShow)
      }

      const usersofgroup = [...currentSelect, user.id]

      const newChat = await createChat(usersofgroup)

      let chatdata = newChat.chatdata

    chatdata.icon = chatdata.icon === null ? "https://cdn-icons-png.flaticon.com/512/681/681494.png" : (!chatdata.avatarURL ? chatdata.icon : chatdata.avatarURL)

    chatdata.avatarURL = chatdata.icon
  ...
  useEffect(() => {
    if(CreateGroupDialogOpen === false){
        setCurrentSelect([])
  ...
  */}
      ...
      <Transition appear show={CreateGroupDialogOpen} as={Fragment}>
            ...
            <div className="flex min-h-full items-center justify-center p-4 text-center">
              ...
              >
                <Dialog.Panel className="dialogpanel fit-content max-w-md transform overflow-hidden p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  >
                    Selecciona usuarios para crear el grupo:
                  </Dialog.Title>
...
<div  onClick={() => addOrRemoveUserToArray(notuser.id)} key={index} className='newChatOnList hover'>
<div className="">
<div className="pfp"><img src={chatdata.avatarURL} alt="" /></div>
    ...
    {currentSelect.length < 2 && currentSelect.length > 0 && <p className='text-center text-red-500'>Mínimo 2 usuarios</p>}
    ...
    {userChats.length <= 1 &&  <p className='text-center textoboton'>No se encontraron usuarios</p>}
                 ...
                 <button
    type="button"
    className={`inline-flex justify-center rounded-md border border-transparent px-10 py-2 text-sm font-medium ${currentSelect.length < 2 || LoadingCreateChat === true ? 'bg-gray-700 text-gray-500 cursor-not-allowed' : 'bg-yellow-300 text-blue-900 hover:bg-yellow-200'}`}
    onClick={createChatFunc}
    disabled={currentSelect.length < 2 || LoadingCreateChat === true}
>
